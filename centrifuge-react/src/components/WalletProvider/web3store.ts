...
/**
 * Modified Web3React store, so it can be used for multichain connectors as well
 * @see {@link https://github.com/Uniswap/web3-react/blob/main/packages/store/src/index.ts}
 */
...
function validateChainId(chainId: number | string, type: 'evm' | 'caip2'): void {
  switch (type) {
    case 'caip2':
      if (typeof chainId !== 'string' || /[-a-z0-9]{3,8}:[-_a-zA-Z0-9]{1,32}/.test(chainId)) {
        throw new Error(`Invalid chainId ${chainId}`)
      }
    case 'evm':
      if (typeof chainId !== 'number' || !Number.isInteger(chainId) || chainId <= 0 || chainId > MAX_SAFE_CHAIN_ID) {
        throw new Error(`Invalid chainId ${chainId}`)
      }
  }
}
...
export function createWeb3ReactStoreAndActions(type: 'evm' | 'caip2' = 'evm'): [Web3ReactStore, Actions] {
  ...
  function update(stateUpdate: Web3ReactStateUpdate): void {
    ...
    if (stateUpdate.chainId !== undefined) {
      validateChainId(stateUpdate.chainId, type)
    }
