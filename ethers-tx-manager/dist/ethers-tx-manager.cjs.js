...
class TransactionManager extends ethers.ethers.Signer {
    constructor(signer, config) {
        super();
        this.latestGasPrices = undefined;
        this.transactions = {};
        this.queue = [];
        ethers.ethers.utils.defineReadOnly(this, 'signer', signer);
    ...
    async process(key, increases = 0) {
        ...
        if (ethers.ethers.BigNumber.from(gasPrice).lt(ethers.ethers.BigNumber.from(request.gasPrice).add(ethers.ethers.BigNumber.from(this.config.minGasPriceIncrease)))) {
