...
const USE_EVM = false
const USE_EXTERNAL_LOANS = false
...
export function CreateAndSetupPool() {
  ...
  if (evmChainId) {
    console.log('addresses', getAddresses(true, evmChainId))
  }

  function getAddresses(useEvm: boolean, evmChainId: number) {
  ...
  const { execute, isLoading } = useCentrifugeTransaction(
    'Create pool',
    () =>
      ([txId, useExternalLoans, useEvm, evmChainId]: [string, boolean, boolean, number], options) => {
        async function doTransactions() {
          ...
          const {
            ...
            navManagerCentAddress,
            feeReceiverCentAddress,
          } = getAddresses(useEvm, evmChainId)
          ...
          const metadata: PoolMetadata = {
            ...
            pool: {
              ...
              links: {
                executiveSummary: {
                  uri: 'ipfs://QmSPNN2QsqBB9MkiifWQi55iVZ2CLicq7X6EezZwB98uLE',
              ...
              useExternalLoans
                ? api.tx.proxy.proxy(
                    ...
                    api.tx.utility.batchAll([
                      ...
                      api.tx.oraclePriceCollection.applyUpdateCollectionInfo(
                        poolId,
                        blake2AsHex(feederChange.toU8a(), 256)
                      ),
                    ])
                  )
                : (null as never),
          ...
          // Invest
          const investTx = api.tx.utility.batchAll([
            api.tx.investments.updateInvestOrder([poolId, juniorTranche], CurrencyBalance.fromFloat(10000, 6)),
            // Send AO proxy some USDC to repay interest
            api.tx.tokens.transfer(aoProxy, { LocalAsset: '1' }, CurrencyBalance.fromFloat(1000, 6)),
          ])
          ...
          const internalAssetPricing: LoanInfoInput = {
            valuationMethod: 'outstandingDebt',
            value: CurrencyBalance.fromFloat(1000, 6),
            advanceRate: Rate.fromPercent(100),
            maxBorrowAmount: 'upToOutstandingDebt',
            maturityDate: new Date('2025-12-31'),
            maturityExtensionDays: null,
            interestRate: Rate.fromPercent(5),
          }
          const [createLoanTx1, createLoanTx2] = await firstValueFrom(
            combineLatest([
              ...
              borrowerCent.pools.createLoan(
                [poolId, nftCollectionId, assetNftId2, useExternalLoans ? oracleAssetPricing : internalAssetPricing],
                {
                  batch: true,
                }
              ),
          ...
          const epochTx = api.tx.proxy.proxy(
            adminProxy,
            undefined,
            api.tx.utility.batchAll([
          ...
          await lastValueFrom(poolManagerCent.wrapSignAndSend(api, epochTx))
          ...
          const borrowTx = api.tx.proxy.proxy(
            aoProxy,
            'Borrow',
            api.tx.loans.borrow(
              poolId,
              ...
              useExternalLoans
                ? {
                    external: {
                      quantity: Price.fromFloat(10),
                      settlementPrice: CurrencyBalance.fromFloat(100.1, 6),
                    },
                  }
                : {
                    internal: CurrencyBalance.fromFloat(1000, 6),
                  }
            )
          )

          const lastResult2 = await lastValueFrom(
            borrowerCent.wrapSignAndSend(api, borrowTx)
          ...
          const repayTx = api.tx.proxy.proxy(
            aoProxy,
            'Borrow',
            api.tx.utility.batchAll([
              api.tx.loans.repay(
                ...
                useExternalLoans
                  ...
                  : {
                      principal: { internal: CurrencyBalance.fromFloat(1000, 6) },
                      interest: CurrencyBalance.fromFloat(9999, 6),
                      unchecked: '0',
                    }
              ),
            ])
          ...
          const lastResult = await lastValueFrom(
            ...
            borrowerCent.wrapSignAndSend(api, repayTx, { onStatusChange: options?.onStatusChange })
          )
  ...
  const { execute: executeFund, isLoading: isLoadingFunding } = useCentrifugeTransaction(
    'Fund accounts',
    (cent) =>
      ([useEvm, evmChainId]: [boolean, number], options) => {
        const {
          poolManagerCentAddress,
          borrowerCentAddress,
          investorCentAddress,
        ...
        const tx = api.tx.utility.batchAll([
          api.tx.balances.transferKeepAlive(poolManagerCentAddress, CurrencyBalance.fromFloat(100000, 18)),
          api.tx.balances.transferKeepAlive(borrowerCentAddress, CurrencyBalance.fromFloat(100000, 18)),
          api.tx.balances.transferKeepAlive(investorCentAddress, CurrencyBalance.fromFloat(100000, 18)),
          api.tx.balances.transferKeepAlive(navManagerCentAddress, CurrencyBalance.fromFloat(100000, 18)),
          api.tx.balances.transferKeepAlive(feeReceiverCentAddress, CurrencyBalance.fromFloat(100000, 18)),
          api.tx.tokens.transfer(investorCentAddress, { LocalAsset: '1' }, CurrencyBalance.fromFloat(1000000, 6)),
        ])
    ...
    <>
      <button
        ...
        onClick={() => {
          ...
          execute([txId, USE_EXTERNAL_LOANS, USE_EVM, evmChainId], undefined, txId)
          updateTransaction(txId, {
      ...
      <button
        ...
        onClick={() => {
          if (!evmChainId) return
          executeFund([USE_EVM, evmChainId])
        }}
      >
        {isLoadingFunding ? 'Funding...' : 'Fund accounts'}
      </button>
    </>
  )
}
