...
export class AgreementController {
  ...
  async redirectToAgreement(@Param() params, @Query() query, @Res({ passthrough: true }) res, @Req() req) {
    ...
    if (!verifiedSession) {
      const returnUrl = CustomPoolIds.includes(params.poolId.trim())
        ? `${origin}onboarding/${params.poolId.trim()}`
        : `${origin}pool/${params.poolId.trim()}/${pool.metadata.slug}/onboarding`
    ...
    const returnUrl = `${config.onboardApiHost}pools/${params.poolId}/agreements/${agreement.id}/callback?origin=${origin}`
  ...
  async agreementCallback(@Param() params, @Query() query, @Res({ passthrough: true }) res) {
  ...
  async postDocusignConnect(@Body() content: DocusignConnectDto): Promise<string> {
    ...
    if (content.status === 'declined') {
      ...
      if (!agreement.declinedAt) {
        ...
        this.mailerService.sendAgreementVoidedEmail(user, pool, agreement.tranche)
      }
      return 'OK'
    }
