...
export const customPools: { [key: string]: string[] } = { mainnet: [RwaMarketKey], goerli: [] }
...
export class PoolService {
  ...
  private async loadFromIPFS() {
    ...
    customPools[network].forEach(async (poolId: string) => {
      ...
      if (profile) {
        poolsWithProfiles[poolId] = { profile, metadata: {}, addresses: {}, network }
      }
    })
