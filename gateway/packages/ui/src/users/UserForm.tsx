...
import { TwoFaType, User, UserWithOrg } from '@centrifuge/gateway-lib/src/models/user'
import { PERMISSIONS } from '@centrifuge/gateway-lib/src/utils/constants'
import { mapSchemaNames } from '@centrifuge/gateway-lib/src/utils/schema-utils'
import { Formik } from 'formik'
import { Box, Button, CheckBox, FormField, Select, Text, TextInput } from 'grommet'
  ...
  render() {
    ...
    return (
      <Box margin={{ vertical: 'medium' }} overflow={'auto'}>
        ...
        >
          {({ values, errors, handleChange, handleSubmit, setFieldValue, dirty }) => (
            <form
              onSubmit={(event) => {
              ...
              <Box gap="medium">
                ...
                {newOrg ? (
                  <FormField label="Organization name" error={errors.organizationName}>
                    <TextInput name="organizationName" value={values!.organizationName} onChange={handleChange} />
                  </FormField>
                ) : (
                  <FormField label="Organization" error={errors.account}>
                    <SearchSelect
                      ...
                      valueLabel={(() => {
                        return values.account ? <p>{values.organizationName + ' / ' + values.account}</p> : undefined
                      })()}
                      onChange={(selected) => {
                ...
                {values.permissions.includes(PERMISSIONS.CAN_MANAGE_DOCUMENTS) && (
                  <>
                    {schemaOptions && schemaOptions.length > 0 ? (
                      <Box margin={{ bottom: 'medium' }}>
                        <FormField label="Document schemas" error={errors!.schemas}>
                          <MultipleSelect
                            closeOnChange={false}
                            labelKey={(item) => {
                      ...
                      <Text color={'status-warning'}>
                        No schemas in the database. Please <Link to={routes.schemas.index}>add schemas</Link>!
                      </Text>
                    )}
                  </>
                )}
                ...
                <Box direction="row" justify={'end'} gap={'medium'}>
                  ...
                  <Button disabled={!dirty} type="submit" primary label={user._id ? 'Update' : 'Create'} />
                </Box>
              </Box>
            </form>
          )}
