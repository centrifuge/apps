...
import { User, UserWithOrg } from '@centrifuge/gateway-lib/src/models/user'
...
it('Should render the create state', () => {
  ...
  const form = mount(<UserForm user={user} schemas={schemas} organizations={organizations} />)
  expect(form.html()).toMatchSnapshot()
})

it('Should render in edit state with all fields including values for schemas input', () => {
  const organizations = [new Organization('My Org', '0x483838'), new Organization('My Second Org', '0x48383833')]
  ...
  const schemas = [{ name: 'option-1' } as Schema, { name: 'option-2' } as Schema]
