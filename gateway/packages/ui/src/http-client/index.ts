...
import {
  MintNftRequest,
  TransferNftRequest,
} from '@centrifuge/gateway-lib/models/nfts';
...
export const httpClient = {
  user: {
    ...
    delete: async (user: User) =>
      instance.delete(`${ROUTES.USERS.base}/${user._id}`),
  ...
  funding: {
    create: async (fundingRequest: FundingRequest) =>
      instance.post(ROUTES.FUNDING.base, fundingRequest),
    sign: async (fundingRequest: FundingSignatureRequest) =>
      instance.post(ROUTES.FUNDING.sign, fundingRequest),
  },
  transferDetails: {
    create: async (transferDetails: TransferDetailsRequest) =>
      instance.post(ROUTES.TRANSFER_DETAILS, transferDetails),
    update: async (transferDetails: TransferDetailsRequest) =>
      instance.put(`${ROUTES.TRANSFER_DETAILS}`, transferDetails),
  },
  schemas: {
    create: async (schema: Schema) => instance.post(ROUTES.SCHEMAS, schema),
    list: async (query = {}) =>
      instance.get(ROUTES.SCHEMAS, { params: { ...query } }),
    getById: async (id): Promise<AxiosPromise<Schema>> =>
      instance.get(`${ROUTES.SCHEMAS}/${id}`),
    update: async (schema: Schema) =>
      instance.put(`${ROUTES.SCHEMAS}/${schema._id}`, schema),
    archive: async (id: string) =>
      instance.put(`${ROUTES.SCHEMAS}/${id}/archive`),
    restore: async (id: string) =>
      instance.put(`${ROUTES.SCHEMAS}/${id}/restore`),
  },
  documents: {
    create: async (document: Document) =>
      instance.post<Document>(ROUTES.DOCUMENTS, document),
    list: async () => instance.get(ROUTES.DOCUMENTS),
    getById: async (id): Promise<Document> =>
      instance.get(`${ROUTES.DOCUMENTS}/${id}`),
    update: async (document: Document) =>
      instance.put(`${ROUTES.DOCUMENTS}/${document._id}`, document),
    //TODO we should not use template as the document _id.
    clone: async (document: Document) =>
      instance.post<Document>(
        `${ROUTES.DOCUMENTS}/${document.template}/clone`,
        document,
      ),
    commit: async (id: string) =>
      instance.put(`${ROUTES.DOCUMENTS}/${id}/commit`),
  },
  nfts: {
    mint: async (payload: MintNftRequest) =>
      instance.post(`${ROUTES.NFTS}/mint`, payload),
    transfer: async (payload: TransferNftRequest) =>
      instance.post(`${ROUTES.NFTS}/transfer`, payload),
  },

  organizations: {
    create: async (organization: Organization) =>
      instance.post(ROUTES.ORGANIZATIONS, organization),
