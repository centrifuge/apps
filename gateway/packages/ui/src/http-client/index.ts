...
export const httpClient = {
  user: {
    ...
    invite: async (user: User, token: string) =>
      instance.post(ROUTES.USERS.invite, user, authHeader(token)),
    update: async (user: User, token: string) =>
      instance.put(ROUTES.USERS.base, user, authHeader(token)),
    delete: async (user: User, token: string) =>
      instance.delete(`${ROUTES.USERS.base}/${user._id}`, authHeader(token)),
    list: async (token: string) =>
      instance.get(ROUTES.USERS.base, authHeader(token)),
  },
  contacts: {
    create: async (contact: Contact, token: string) =>
      instance.post(ROUTES.CONTACTS, contact, authHeader(token)),
    list: async (token: string) =>
      instance.get(ROUTES.CONTACTS, authHeader(token)),
    update: async (contact: Contact, token: string) =>
      instance.put(
        `${ROUTES.CONTACTS}/${contact._id}`,
        contact,
        authHeader(token),
      ),
  },
  ...
  schemas: {
    create: async (schema: Schema, token: string) =>
      instance.post(ROUTES.SCHEMAS, schema, authHeader(token)),
  ...
  documents: {
    ...
    list: async (token: string) =>
      instance.get(ROUTES.DOCUMENTS, authHeader(token)),
  ...
  organizations: {
    ...
    list: async (token: string) =>
      instance.get(ROUTES.ORGANIZATIONS, authHeader(token)),
    update: async (organization: Organization, token: string) =>
      instance.put(
        `${ROUTES.ORGANIZATIONS}/${organization._id}`,
        organization,
        authHeader(token),
      ),
  },
