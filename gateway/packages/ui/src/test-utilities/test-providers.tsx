...
export const withApplicationContext = (Component, user: User = defaultUser, setUser = () => {}) => {
  return <AppContext.Provider value={{ user, setUser }}>{Component}</AppContext.Provider>
}
...
export const withAllProvidersAndContexts = (Component, user: User = defaultUser, setUser = () => {}) => {
  return withAxis(withApplicationContext(withNotificationContext(Component), user, setUser))
}
