      ...
      >
        {({ values, errors, handleChange, handleSubmit }) => (
          ...
          >
            <Box gap="small">
              {info && <p>{info}</p>}
              <FormField label="Security code" info={'Enter generated security code for Gateway'} error={errors!.token}>
                <TextInput name="token" value={values.token || ''} onChange={handleChange} />
              </FormField>

              {error && <Text color={'status-error'}>Failed to validate code!</Text>}
