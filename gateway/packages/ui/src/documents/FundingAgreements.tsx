...
import { FundingAgreement, FundingRequest } from '@centrifuge/gateway-lib/src/models/funding-request'
import { canSignFunding, User } from '@centrifuge/gateway-lib/src/models/user'
import { getAddressLink } from '@centrifuge/gateway-lib/src/utils/etherscan'
import { extractDate, formatCurrency, formatPercent } from '@centrifuge/gateway-lib/src/utils/formaters'
...
export const FundingAgreements: FunctionComponent<Props> = (props) => {
  ...
  const { onAsyncStart, onAsyncComplete, onAsyncError, document, user, contacts, viewMode } = {
  ...
  const fundingActions = !viewMode
    ? [
        <Button
          key="create-funding-agreement"
          onClick={() => openModalInEditMode(new FundingAgreement())}
          icon={<Currency />}
          plain
    ...
    },
    {
      ...
      render: (datum) => (
        <Box className={'actions'} direction="row" gap="small">
          <Anchor label={'View'} onClick={() => openModalInViewMode(datum)} />
          {datum.status !== FUNDING_STATUS.ACCEPTED && canSignFunding(user, datum) && (
            <Anchor label={'Sign'} onClick={() => singFundingAgreement(datum)} />
          )}
        </Box>
      ),
    },
  ...
  return (
    <>
      ...
      <Section title="Funding Agreements" actions={fundingActions}>
        ...
        {!mappedToSortable.length && <Paragraph color={'dark-2'}>There are no funding agreements yet.</Paragraph>}
      </Section>
    </>
  )
}
