...
export const FundingAgreements: FunctionComponent<Props> = props => {
  const [
    { modalOpened, selectedFundingAgreement, isViewMode },
    setState,
  ] = useMergeState<State>({
    modalOpened: false,
    ...
    },
    {
      ...
      render: datum => (
        <Box className={'actions'} direction="row" gap="small">
          ...
          {datum.status !== FUNDING_STATUS.ACCEPTED &&
            canSignFunding(user, datum) && (
              <Anchor
                label={'Sign'}
                onClick={() => singFundingAgreement(datum)}
              />
            )}
        </Box>
      ),
      ...
      <Section title="Funding Agreements" actions={fundingActions}>
        ...
        {!mappedToSortable.length && (
          <Paragraph color={'dark-2'}>
            There are no funding agreements yet.
          </Paragraph>
        )}
      </Section>
    </>
