...
import { AttrTypes, Schema } from '@centrifuge/gateway-lib/src/models/schema'
import { HARDCODED_FIELDS } from '@centrifuge/gateway-lib/src/utils/constants'
import { applySchemaRules, revertSchemaRules } from '@centrifuge/gateway-lib/src/utils/document-mutations'
...
export class DocumentForm extends React.Component<Props, State> {
  ...
  constructor(props) {
    ...
    const found: Schema | undefined =
      ...
      schemas.find((s) => {
        return (
          document && document.attributes && document.attributes._schema && s.name === document.attributes._schema.value
        )
      })
    const { validationSchema, clone } = this.generateDefaultValuesAndValidation(found, document)
  ...
  selectSchema = (selected) => {
    const { validationSchema, clone } = this.generateDefaultValuesAndValidation(selected, this.props.document)
  ...
  generateDefaultValuesAndValidation = (schema: Schema | undefined, document: Document) => {
  ...
  render() {
    const { submitted, selectedSchema, sectionGap, columnGap, document, validationSchema } = this.state
