...
Given(
  'there is no outstanding order or collection for the {tranche} tranche',
  async function (this: CentrifugeWorld, tranche: Tranche) {
...
When(
  'I {order} {int} DAI for {tranche}',
  async function (this: CentrifugeWorld, order: Order, amount: number, tranche: Tranche) {
...
Then(
  'there is an outstanding order for the {tranche} tranche',
  async function (this: CentrifugeWorld, tranche: Tranche) {
    const hasOutstandingOrder = await isElementVisible(
      this.currentPage,
      tinlake(
