  ...
  repay = async () => {
    try {
      ...
      transactionSubmitted &&
        transactionSubmitted(
          'Repayment initiated. Please confirm the pending transactions. ' + 'Processing may take a few seconds.'
        )
        ...
        <Box gap="medium">
          <FormField label="Repay amount">
            <NumberInput
              ...
              suffix=" DAI"
              precision={18}
              onValueChange={({ value }) => this.setState({ repayAmount: displayToBase(value, 18) })}
              disabled
            />
          </FormField>
        </Box>
