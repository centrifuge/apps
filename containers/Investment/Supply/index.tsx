...
class InvestorSupply extends React.Component<Props, State> {
  ...
  supply = async () => {
    ...
    transactionSubmitted &&
      transactionSubmitted(
        'Investment initiated. Please confirm the pending transactions. ' + 'Processing may take a few seconds.'
      )
    try {
      await ensureAuthed!()
        ...
        <Box gap="medium">
          <FormField label="Investment amount">
            <NumberInput
              ...
              suffix=" DAI"
              precision={18}
              onValueChange={({ value }) => this.setState({ supplyAmount: displayToBase(value, 18) })}
            />
          </FormField>
        </Box>
        <Box align="start">
