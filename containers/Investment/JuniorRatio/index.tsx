...
class JuniorRatio extends React.Component<Props, State> {
  ...
  setMinJuniorRatio = async () => {
    ...
    transactionSubmitted &&
      transactionSubmitted(
        'Setting mininum TIN ratio initiated. Please confirm the pending transactions. Processing may take a few seconds.'
      )
    try {
          ...
          <Box basis={'1/3'}>
            <FormField label="Min TIN ratio">
              <NumberInput
                ...
                precision={2}
                onValueChange={({ value }) => this.setState({ minJuniorRatio: displayToBase(value, 27) })}
              />
            </FormField>
          </Box>
          <Box align="start">
