...
class Dashboard extends React.Component<Props> {
  ...
  updateColleteralTimeSeriesData = async (period: string) => {
    ...
    const updatedColleteralValueTimeSeriesData = timeSeriesData.reduce((colleteralValueTimeSeriesData: TimeSeriesData, entry: TinlakeEventEntry) => {
      const dateLabel = (new Date(parseInt(entry.timestamp))).toLocaleDateString("en-US", {month: "short", day: "numeric"})
      colleteralValueTimeSeriesData.labels.push(dateLabel)
      colleteralValueTimeSeriesData.xValues[0].data.push((parseInt(entry.total_debt) / Math.pow(10, 18)).toFixed(2))
      colleteralValueTimeSeriesData.xValues[1].data.push((parseInt(entry.total_value_of_nfts) / Math.pow(10, 18)).toFixed(2))
